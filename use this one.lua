...
-- Apply Speed and Jump Power in loop
spawn(function()
    while Character do
        wait(0.1)
        if Humanoid and Humanoid.Health > 0 then
            ...
            -- Apply Jump Power (using AssemblyLinearVelocity for jump)
            if Humanoid:GetState() == Enum.HumanoidStateType.Jumping then
                RootPart.AssemblyLinearVelocity = RootPart.AssemblyLinearVelocity + Vector3.new(0, JumpPowerValue / 10, 0)
            end
        end
    end
end)
